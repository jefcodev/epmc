...
(function($) {
	...
	$(function () {
		...
		$('#collapse-init').click(function () {
			if (active) {
				...
				$(this).text('Cerrar todos');
			} else {
				active = true;
				$('.panel-collapse').collapse('hide');
				$('.panel-title').attr('data-toggle', 'collapse');
				$(this).text('Abrir todos');
			}
		});
    ...
    $.get(window.location.origin + '/google_stats', function(data) {

        $('#visitas-totales').html(data.users_total);
        $('#visitas-mensuales').html(data.users_month);
        $('#visitas-totales, #visitas-mensuales').counterUp({
            delay: 8,
            time: 1400
        });
    });
