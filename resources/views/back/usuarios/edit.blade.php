          ...
          <br>
            <form id="form_iconic_validation" action="{{ route('usuarios.update',$usuario->id) }}" method="post" enctype="multipart/form-data">
              ...
              <div class="form-group">
                ...
                <span class="help">eg: "1801234567"</span>
                <div class="input-with-icon  right">
                  <i class=""></i>
                  <input type="text" name="cedula" id="cedula" value="{{ $usuario->cedula }}" class="form-control" required>
                </div>
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <div class="input-with-icon  right">
                  <i class=""></i>
                  <select name="roles_id[]" multiple class="select2 form-control" data-init-plugin="select2">
                    @foreach($roles as $key => $role )
                      <option value="{{ $key }}" @if($usuario->hasRole($role)) selected @endif >{{ $role }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
