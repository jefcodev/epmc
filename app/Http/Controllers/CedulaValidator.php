...
class CedulaValidator extends Controller
{
    /**
     * Error
     *
    ...
    public function validar($cedula, Request $request){
    	 
    	try {
	    	if(strlen($cedula<10)){
	    		return response()->json(false);
	    	}elseif(strlen($cedula)==10){
	    		return response()->json($this->validarCedula($cedula));
	    	}elseif(strlen($cedula)==13){
	    		return response()->json($this->validarRucPersonaNatural($cedula)||$this->validarRucSociedadPrivada($cedula)||$this->validarRucSociedadPublica($cedula));
	    	}
	    	return response()->json(false);
	    }catch(Exception $ex){
	    	return response()->json(false);
    ...
    /**
     * Devuelve un JSOn con la respuesta de ecuadorlegalonline
     */
    public function placa($placa){
    	$url = "http://www.ecuadorlegalonline.com/modulo/sri/matriculacion/consultar-vehiculo.php?placa=".$placa;
    	//  Initiate curl
		$ch = curl_init();
		...
		//dd($result, json_decode($result), json_decode($result,true));

		return response()->json((json_decode(json_decode($result))));
    }
